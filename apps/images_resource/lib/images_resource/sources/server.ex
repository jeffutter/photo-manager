defmodule ImagesResource.Sources.Server do
  @callback refresh(state :: term) :: {:ok, new_state} | {:error, reason} when reason: term, new_state: term
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      ...
      @type t :: %{tree: Gallery.t | nil, name: atom() | nil, sync_targets: [atom()], refresh_period: integer(), options: Keyword.t}
      ...
      def start_link(name, opts \\ []) do
        ...
        GenServer.start_link(__MODULE__, %__MODULE__{name: name, sync_targets: sync_targets, tree: tree, refresh_period: refresh_period, options: opts}, name: name)
      end
