defmodule PhotoManagementApi.Web.Resolver.Image do
  ...
  def rate_image(%{slug: slug, rating: rating}, %{context: %{current_user: %PhotoManagementApi.User{id: user_id}}}) do
    r = {user_id, slug}
        |> find_or_create_rating

    updated = PhotoManagementApi.Rating.changeset(r, %{slug: slug, user_id: user_id, rating: rating})
  ...
  def find_or_create_rating({user_id, slug}) do
    PhotoManagementApi.Repo.get_by(PhotoManagementApi.Rating, user_id: user_id, slug: slug) || %PhotoManagementApi.Rating{slug: slug, user_id: user_id}
  end
end
